...
class WeightDataFetcher:
    def __init__(self, account=None, password=None, nickname=None):
...
class XHSDataFetcher:
    def __init__(self, server_address, link, base_path):
        self.server_address = server_address
        self.link = link
        self.base_path = base_path

    def fetch_data(self, attempts=1):
        server = f"{self.server_address}/xhs/"
        data = {"url": self.link, "download": True}
        response = requests.post(server, json=data)

        if response.status_code != 200:
            print("请求失败，状态码:", response.status_code)
            if attempts < 10:
                return self._try_to_get_data_again(attempts + 1)
            else:
                print(f"此链接疑似无效1:{self.link}")
                return ""
        ...
        if response_data is None:
            if attempts < 10:
                ...
                print(f"此链接疑似无效2:{self.link}")
                return "", ""
        response_data["下载地址"] = self.get_new_download_links(response_data)
        return response_data

    def _try_to_get_data_again(self, attempts):
        return self.fetch_data(attempts)

    def get_new_download_links(self, response_data):
        work_id = response_data["作品ID"]
        saved_paths_pic = []
        saved_paths_video = []
        for i, link in enumerate(response_data["下载地址"], start=1):
            pic_link = f"{self.base_path}/{work_id}_{i}.webp"
            video_link = f"{self.base_path}/{work_id}.mp4"

            if "video" in link:
                saved_paths_video.append(video_link)
            else:
                saved_paths_pic.append(pic_link)

        if saved_paths_pic:
            final_link = ",".join(saved_paths_pic)

        else:
            final_link = ",".join(saved_paths_video)
        return final_link

    @staticmethod
    def convert_hashtags_and_mentions_to_links(text):
        pattern = r"#(.*?)\[话题\]#|@\w+"

        def replace_with_link(match):
            if match.group(1):
                hashtag = match.group(1).strip()
                keyword_encoded = urllib.parse.quote(hashtag)
                return f"[# {hashtag}](https://www.xiaohongshu.com/search_result/?keyword={keyword_encoded}&type=54&source=web_note_detail_r10)"
            elif match.group(0).startswith("@"):
                username = match.group(0)[1:]
                username_encoded = urllib.parse.quote(username)
                return f"[@ {username}](https://www.xiaohongshu.com/search_result?keyword={username_encoded}&source=web_user_page&search_type=user&type=51)"

        converted_text = re.sub(pattern, replace_with_link, text).replace("\n", "\n\n")
